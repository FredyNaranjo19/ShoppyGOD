...
//************************ FUNCIONES DE LOTES DE PRODUCTO ************************************
	...
	public function ajaxCrearLote(){
		...
		$datos = ["costo" => round($costoPromedio,6), "modelo" => $this -> loteCrearCodigo];
	...
	public function ajaxEditarLote(){
				...
				*/
				...
				if ($respuestaLoteActual["costo_prom_ant"] !== 0  ) { //&& $newStockDisp >= 0 && $loteCostoInput !== $respuestaLoteActual["costo"] 
					...
					if ($newStockDisp >= 0) {
						...
						// $respuesta += ["newSD" => $newStockDisp, "newSG" => $newStockGral];
					...
					// $accion .= "modCosto";
					$respuesta .= "modCosto";
					// $respuesta += ["lotes" => $loteCant, "valorInv" => $valorInventario, "valorEnt"=> $valorEntrada,"newCosto" => $newCosto];
				}else{

					// $accion = "error";
					$respuesta = "error";

					// if ($loteCostoInput !== $respuestaLoteActual["costo"]) {
					// 	# code...
					// }
				}
		...
		// $respuestafin =  ["accion" => $accion, "respuesta" => $respuesta];
