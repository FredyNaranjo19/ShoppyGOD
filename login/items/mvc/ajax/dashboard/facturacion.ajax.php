...
class AjaxFacturacion{
	...
	/*==================================================================
	=            MOSTRAR INFORMACION DE EMPRESA FACTURACION            =
	==================================================================*/
	
	public $busquedaRFCfacturacion;
	public function ajaxMostrarInformacionEmpresa(){

		$tabla = "modulo_facturas_datos";
		$item = "rfc";
		$valor = $this -> busquedaRFCfacturacion;
	...
	/*=====  End of MOSTRAR INFORMACION DE EMPRESA FACTURACION  ======*/
	...
	public function ajaxAgrupacionPedidos(){
		...
		foreach ($folio as $key => $value) {
			...
			if ($key == 0) {
				...
				$consulta .= " d.folio = '".$value["folio"]."'";

			} else {

				$consulta .= " OR d.folio = '".$value["folio"]."'";

			}
			
		}

		$respuesta = ModeloPedidos::mdlMostrarAgrupacionFolios($consulta, $empresa);
	...
	public function ajaxRealizarFactura(){
		...
		foreach ($agrupacion as $key => $value) {
			...
			$datos['conceptos'][$key]['descripcion'] = $value["producto"];//"PRODUCTO DE PRUEBA 1";
			$datos['conceptos'][$key]['valorunitario'] = $value["precio_sin_iva"];//99.00;
			$datos['conceptos'][$key]['importe'] = $value["subtotal"];//99.00;
			$datos['conceptos'][$key]['ClaveProdServ'] = $value["clavePS"];//'01010101';
			$datos['conceptos'][$key]['ClaveUnidad'] = $value["claveUnidad"];//'ACT';

			$datos['conceptos'][$key]['Impuestos']['Traslados'][0]['Base'] = $value["subtotal"];//99.00;
			...
			$datos['conceptos'][$key]['Impuestos']['Traslados'][0]['Importe'] = $value["iva"];//15.84;
		...
		$datos['impuestos']['translados'][0]['importe'] = $this -> ivaFactura;//47.68;
		$datos['impuestos']['translados'][0]['TipoFactor'] = 'Tasa';


		$datos['impuestos']['TotalImpuestosTrasladados'] = $this -> ivaFactura;//47.68;
		...
		if ($resPDF == "OK") {
			...
			foreach ($folios as $key => $valueFolio) {
				...
				$respuestaPedido = ModeloPedidos::mdlModificacionFactura($tablaPedido, $datosPedido);
...
// 		$datos["produccion"]= "NO";
	...
	/*==============================================================
	=            CONFIGURACION DE FACTURACION FUNCTIONS            =
	==============================================================*/
		...
		public $crearSerieFactura;
		public $crearSignificadoFactura;
		public $crearFolioInicialFactura;
		
		public function ajaxConfSeriesFacturacion(){

			$tabla = "modulo_facturas_series";
			$datos =  array("id_empresa" => $_SESSION["idEmpresa_dashboard"],
							"serie" => $this -> crearSerieFactura,
							"significado" => $this -> crearSignificadoFactura,
							"folio_inicial" => $this -> crearFolioInicialFactura);

			$respuesta = ModeloFacturacion::mdlCrearConfiguracionSerieFactura($tabla, $datos);
		...
		public $idEditarSerieFacturacion;
		public $serieEditarSerieFacturacion;
		public $nombreEditarSerieFacturacion;
		public function ajaxConfSeriesEditarFacturacion(){

			$tabla = "modulo_facturas_series";
			$datos =  array("id_modulo_facturas_series" => $this -> idEditarSerieFacturacion,
							"serie" => $this -> serieEditarSerieFacturacion,
							"significado" => $this -> nombreEditarSerieFacturacion);

			$respuesta = ModeloFacturacion::mdlEditarConfiguracionSerieFactura($tabla, $datos);
...
if (isset($_POST["busquedaRFCfacturacion"])) {
	$mostrarDatosFactura = new AjaxFacturacion();
	$mostrarDatosFactura -> busquedaRFCfacturacion = $_POST["busquedaRFCfacturacion"];
	$mostrarDatosFactura -> ajaxMostrarInformacionEmpresa();
}
...
if (isset($_POST["agrupacionFolios"])) {
	$agrupacion = new AjaxFacturacion();
...
if (isset($_POST["razonEmisor"])) {
	$crearFactura = new AjaxFacturacion();
...
if (isset($_POST["cancelacionFactura"])) {
	$cancelarFactura = new AjaxFacturacion();
...
==============================================================*/
	...
	if (isset($_POST["crearSerieFactura"])) {
		$crearConfSeries = new AjaxFacturacion();
		$crearConfSeries -> crearSerieFactura = $_POST["crearSerieFactura"];
		$crearConfSeries -> crearSignificadoFactura = $_POST["crearSignificadoFactura"];
		$crearConfSeries -> crearFolioInicialFactura = $_POST["crearFolioInicialFactura"];
		$crearConfSeries -> ajaxConfSeriesFacturacion();
	}
	...
	if (isset($_POST["idEditarSerieFacturacion"])) {
		$editarConfSeries = new AjaxFacturacion();
		$editarConfSeries -> idEditarSerieFacturacion = $_POST["idEditarSerieFacturacion"];
		$editarConfSeries -> serieEditarSerieFacturacion = $_POST["serieEditarSerieFacturacion"];
		$editarConfSeries -> nombreEditarSerieFacturacion = $_POST["nombreEditarSerieFacturacion"];
		$editarConfSeries -> ajaxConfSeriesEditarFacturacion();
	}
